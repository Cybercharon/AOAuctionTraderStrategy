...
class Resources:
    # Make a function to return all the relevant info
    def __init__(self):
        query_string = ''
        for i in range(1,7):
            # query_string += f'T{i}_ROCK,'
            query_string += f'T{i}_ROCK,T{i}_WOOD,T{i}_ORE,T{i}_FIBER,T{i}_HIDE,'
        resource_list = requests.get(
            f'https://www.albion-online-data.com/api/v2/stats/Prices/{query_string}.json').json()
        for item in resource_list:
            print(item)

class RefinedResources:
    # Make a function to return all the relevant info
    def __init__(self):
        query_string = ''
        for i in range(1,7):
            # query_string += f'T{i}_ROCK,'
            query_string += f'T{i}_STONEBLOCK,T{i}_PLANKS,T{i}_METALBAR,T{i}_CLOTH,T{i}_LEATHER,'
        resource_list = requests.get(
            f'https://www.albion-online-data.com/api/v2/stats/Prices/{query_string}.json').json()
        for item in resource_list:
            print(item)
...
class UpgradeParts:
    def __init__(self, level, city='Bridgewatch'):
        query_string = f'{level}_RUNE, {level}_SOUL, {level}_RELIC'
        upgradeParts_list = requests.get(
            f'https://www.albion-online-data.com/api/v2/stats/Prices/{query_string}.json?locations={city}').json()
...
class AlgoTrade:
    ...
    def make_base_item_list(self, item_list=[]):
        ...
        base_item_list = requests.get(
            f'https://www.albion-online-data.com/api/v2/stats/Prices/{query_string}.json?locations={self.city}').json()
        return base_item_list

    def make_max_item_dict(self, item_list=[]):
        item_dict = dict()
        max_query_string = '@3,'.join(item_list)
        max_item_list = requests.get(
            f'https://www.albion-online-data.com/api/v2/stats/Prices/{max_query_string}.json?locations={self.city}').json()
    ...
    #######
    #######
    #### Build a function to buy low and sell high on resources
    #######
    ...
    def full_upgrade(self, item_list=[]):
        ...
        for item in base_item_list:
            ...
            if profit > 0:
                profit_dict.update(
                    {"Item": joint_name, "Base": price, "Upgrade Cost": total_upgrade_cost, "Max Sell": max_price,
                     "Profit": profit})
                profit_list.append(profit_dict)
                print({"Item": joint_name, "Base": price, "Upgrade Cost": total_upgrade_cost, "Max Sell": max_price,
                       "Profit": profit})
...
if __name__ == '__main__':
    ...
    trader.full_upgrade(
        item_list=['T4_OFF_BOOK', 'T4_OFF_DEMONSKULL_HELL', 'T4_OFF_LAMP_UNDEAD', 'T4_OFF_JESTERCANE_HELL',
                   'T4_OFF_HORN_KEEPER', 'T4_OFF_ORB_MORGANA', 'T4_OFF_TORCH', 'T4_OFF_TOTEM_KEEPER'])
